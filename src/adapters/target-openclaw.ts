// src/adapters/target-openclaw.ts
...
export const targetOpenclaw: TargetAdapter = {
  toolId: 'openclaw',
  label: 'OpenClaw',
  ...
  write(entries: MemoryEntry[], basePath: string): MigrationResult {
    ...
    try {
      for (const entry of entries) {
        const typeDir = path.join(resolvedPath, 'workspace', 'memory', 'migrated', entry.type);
        fs.mkdirSync(typeDir, { recursive: true });

        const filename = `${slugify(entry.sourceTool)}-${slugify(entry.title)}.md`;
      ...
      result.errors.push(`Failed to write OpenClaw memory: ${err}`);
    }
