...
export const targetHermes: TargetAdapter = {
  toolId: 'hermes',
  label: 'Hermes',
  defaultPath: '~/.hermes/',
  ...
  write(entries: MemoryEntry[], basePath: string): MigrationResult {
    ...
    try {
      for (const entry of entries) {
        const memDir = path.join(resolvedPath, 'memories', 'migrated', slugify(entry.sourceTool));
      ...
      result.errors.push(`Failed to write Hermes memories: ${err}`);
    }
