// src/adapters/target-roo-code.ts
...
export const targetRooCode: TargetAdapter = {
  toolId: 'roo-code',
  label: 'Roo Code',
  defaultPath: '~/.roo-code/',
  ...
  write(entries: MemoryEntry[], basePath: string): MigrationResult {
    ...
    try {
      for (const entry of entries) {
        const ruleDir = path.join(resolvedPath, 'rules', slugify(entry.sourceTool));
      ...
      result.errors.push(`Failed to write Roo Code rules: ${err}`);
    }
