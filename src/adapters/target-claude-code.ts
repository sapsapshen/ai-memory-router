// src/adapters/target-claude-code.ts
...
export const targetClaudeCode: TargetAdapter = {
  toolId: 'claude-code',
  label: 'Claude Code',
  defaultPath: '~/.claude/',
  ...
  write(entries: MemoryEntry[], basePath: string): MigrationResult {
    ...
    try {
      for (const entry of entries) {
        const typeDir = path.join(resolvedPath, 'memories', entry.type);
      ...
      result.errors.push(`Failed to write Claude Code memories: ${err}`);
    }
