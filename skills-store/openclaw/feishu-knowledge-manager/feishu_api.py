...
class FeishuAPI:
    ...
    def add_document_to_knowledge_base(self, doc_token, kb_token):
        """将文档添加到知识库分类节点
        
        Args:
            doc_token: 文档token (document_id)
            kb_token: 知识库分类节点token
        ...
        data = {
            "node_type": "doc",
            "doc_id": doc_token,
            "parent_node_token": kb_token
        }
        result = self._request("POST", "/knowledge/v1/nodes", data)
        
        if result.get("code") == 0:
            node = result.get("data", {}).get("node", {})
            print(f"[SUCCESS] Document added to knowledge base: {node.get('node_token')}")
        else:
            print(f"[WARN] Could not add to knowledge base: {result.get('msg')} (code: {result.get('code')})")
